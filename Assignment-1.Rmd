...
subtitle: 'Due: Tuesday 26th January 2016 @ 2pm'
...
## Introduction
This assignment tests your ability to create a Vagrant managed VM running a  Python based client/server application that uses the ZeroMQ messaging library.

This project should be contained in a Vagrant based development environment as described below and also needs to be hosted in a private Git repository on www.bitbucket.org.

## Requirements
### Project Setup

*	You are required to create a project folder called "K00000000_CNSM4_DSCC_Ass01_2015".

*	This folder should contain a Vagrant File and all the Python and other code that you have developed. 

*	You area also required to publish your project to a private Git based repository in https://bitbucket.org/ and give full access to your Lecturer (https://bitbucket.org/johnfjennings/ ). The project must be named K00000000_CNSM4_Ass01
...
*	You must include a __README.md__ file in your project folder that includes:
	...
	-	NOTE this file should be formatted in the Markdown format (see https://confluence.atlassian.com/display/BITBUCKET/_MarkdownOverview )

### Vagrant Specification

*	You are required to build your own Vagrantfile configuration to achieve the following specification:

	-	The VM should be based on the __ubuntu/trusty32__ box
	-	The following packages should be installed if not already pre-installed:
		...
		-	Python 3.4
		- ZeroMQ
	-	Port 3456 on the VM should be forwarded to port 1234 on the host
...
* You are asked to create a client application that complies with the following specification:
	1.	Name the file __client.py__
	2.	The client application should be run from the host computer and not from the Vagrant VM 
	3.	The client application should connect to a socket listening on port 1234 on the host computer 
	4.	The client application should accept a single string parameter and send this string parameter to the client as a ZeroMQ message. Suggested usage is as follows:
```bash
$ python client.py --message="This is a sample message"
```
